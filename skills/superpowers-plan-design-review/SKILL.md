---
name: superpowers-plan-design-review
description: >
  Designer's eye plan review. Rates each design dimension 0-10, explains what would
  make it a 10, then fixes the plan to get there. Works in plan mode. For live site
  visual audits, use /superpowers-design-review. Use when asked to "review the design plan",
  "design critique", or "check the UX before building".
...
> **DEPRECATED** — use `/plan-design-review` instead. This skill is a legacy alias and will be removed in a future cleanup.
# Plan Design Review — Designer's Eye

**Purpose:** Review a plan before implementation. Rate each design dimension 0-10,
explain what would make it a 10, then fix the plan to get there.

**Output:** An improved plan with design decisions added, not a document about the plan.
...
- Check `~/.claude/.context/reviews.jsonl` for prior design reviews.
...
### UI Scope Detection
Analyze the plan. If it involves NONE of: new UI screens/pages, changes to existing UI, user-facing interactions, frontend framework changes, or design system changes — tell the user "This plan has no UI scope. Design review isn't applicable." and exit early.
...
3. **Hierarchy as service** — "What should the user see first, second, third?" Respecting their time.
...
- If no DESIGN.md: "No design system found. Recommend running /superpowers-office-hours first to establish design direction, then we continue."
...
- **APP UI** (workspace-driven, data-dense, task-focused) → apply App UI Rules
...
...
```
Each decision = one AskUserQuestion with recommendation + alternatives. Edit the plan with each decision as it's made.
...
1. **Re-ground:** State the project, branch, and current design gap.
2. **Simplify:** Plain English. No jargon. Concrete examples. Say what the user WILL experience if unresolved.
3. **Recommend:** `RECOMMENDATION: Choose [X]` — prefer the complete option.
4. **Options:** Lettered (A, B, C). Show effort where relevant.
5. **One decision per question.** Never batch.

**Label with issue NUMBER + option LETTER** (e.g., "3A", "3B").

**Escape hatch:** If a section has no issues, say so and move on. If a gap has an obvious fix, state what you'll add and move on — don't waste a question on it.
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual AskUserQuestion. Never batch TODOs. Never silently skip this step.

For design debt: missing a11y, unresolved responsive behavior, deferred empty states. Each TODO gets:
- **What:** One-line description
- **Why:** The concrete problem it solves
- **Pros/Cons:** Cost and benefit
- **Context:** Enough detail for someone to pick it up in 3 months
- **Depends on:** Any prerequisites

Options: **A)** Add to TODOS.md **B)** Skip **C)** Build it now in this PR instead
...
If all passes 8+: "Plan is design-complete. Run /superpowers-design-review after implementation for visual QA."
If any below 8: note what's unresolved and why (user chose to defer).

### Unresolved Decisions
If any AskUserQuestion went unanswered, note it here. Never silently default to an option.

---

## Review Log

After completion, persist the review result:

```bash
mkdir -p ~/.claude/.context
echo '{"skill":"plan-design-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"'"$(git rev-parse --short HEAD 2>/dev/null || echo 'unknown')"'"}' >> ~/.claude/.context/reviews.jsonl
```
...
- If UI scope was detected and no eng review exists: "Consider running /superpowers-plan-eng-review next to validate the architectural implications of these design decisions."
- If design score started below 4/10 and significant product direction gaps were found: "This design review surfaced fundamental questions about whether we're solving the right problem. Consider /superpowers-office-hours before proceeding."
- Otherwise: "All relevant design reviews complete. Run /superpowers-executing-plans when ready to implement."

---

## Completion Status

- **DONE** — All passes complete, plan updated, review logged
- **DONE_WITH_CONCERNS** — Completed with unresolved decisions the user chose to defer
- **BLOCKED** — No UI scope detected or no plan file found
