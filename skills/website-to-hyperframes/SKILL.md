...
## Product Promo Video Pattern

When the request is specifically a **product intro or promotional video** (not just any site capture), apply this additional layer on top of the 7-step workflow.

### Before Step 3 (Script): select a framework

Do not start writing the script until you have explicitly chosen a narrative framework:

| If the viewer is... | Use |
|---|---|
| Seeing the product for the first time | **AIDA** |
| Aware of the problem but not the solution | **PAS** or **BAB** |
| Warm, ready to convert | **Hook-Story-Offer** |
| On social, needs scroll-stopping | **Hook-Body-CTA** |

State the chosen framework at the top of `SCRIPT.md`.

### Step 3 script enforcement for product promo

The script **must** include, in order:
1. **Hook line** — one sentence, works silent, no logo mention, max 3 seconds when spoken
2. **Problem statement** — specific and relatable (not "many people struggle with X")
3. **Solution reveal** — benefit-led ("here's what changes") before feature-led ("here's how it works")
4. **Feature beats** — max 3–4 features; each beat = outcome first, then mechanism
5. **Social proof beat** — one specific metric or quote (not vague: "loved by thousands")
6. **CTA** — one action, first-person verb, specific

**Hook test:** Read the first line aloud. Could it appear as a text overlay and stop a scroll without any voiceover? If no — rewrite it.

**CTA test:** Is it one action? Is it first-person? Is it specific? If any answer is no — rewrite it.

### Ideal lengths to propose (product promo)

| Target platform | Propose |
|---|---|
| Landing page | 60–90s hero |
| Instagram Reels | 15–30s |
| TikTok | 15–30s |
| LinkedIn | 30–60s |
| YouTube pre-roll | 6–15s bumper + 30s full |

If the user hasn't specified: default to 30s for social, 60s for landing page. Build the 30s first — it is the harder constraint. The 60s is an expansion of the same story.
