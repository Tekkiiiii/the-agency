...
description: >
  ...
  Invoke as /dept-save-state [dept-slug] or /dept-save-state all.
...
The departments and their paths (relative to `{agency-root}/agents/`):
...
| Argument | Action |
|---|---|
| `all` | Save all departments, in parallel via subagents |
...
2. If the lesson is cross-department, also note it in `{agency-root}/memory/lessons/` with a reference
