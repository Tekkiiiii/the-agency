...
- Dept-Coord = "DC-tst-{d3-name}-{pun}" (e.g. DC-tst-qa-Sentinel) — D3 track owner
...
2b. Read your scoped structure file (provided by Dept Head in spawn prompt):
    `{agency-root}/agents/testing/state/coords/dc-{name}-structure.md`
...
3b. Write your D4-D6 task structure back to the master dev-plan:
    `{agency-root}/agents/testing/state/dev-plan.md` — append under your D3 section.
4. APPROACH GATE — classify each D6 task as TIER_A or TIER_B before spawning. This gate
   ...
   {agency-root}/agents/testing/scratch/members/member-{id}-{pun}-checkpoint.md and
...
5. QA GATE — Member review (MANDATORY):
   ...
   d. PROGRESS REPORT TO DEPT HEAD (after each Member ACK):
      Send to "testing-lead" via SendMessage:
      ```
      DC-tst-{name}: PROGRESS {completed}/{total} tasks
...
DC MUST notify "testing-lead" via SendMessage before stopping.
...
spawn_id=$(bash ~/.claude/hooks/lib/log-spawn-from-agent.sh \
  --parent-agent "DC-tst-{d3-name}-{pun}" \
