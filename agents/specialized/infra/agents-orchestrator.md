...
## Spawned Agent Permissions (apply to ALL spawns below)

Every agent you spawn via the Agent tool MUST receive this preamble at the start of
their prompt, before the task-specific instructions:

```
Project working directory: {actual absolute path}
You have full read/write/create access to the project directory and all its files
and subdirectories. Use Read, Edit, Write, Bash, Glob, Grep, Agent, SendMessage,
and any other tool you need without asking for permission.

Relevant project context:
[embed relevant project CLAUDE.md, ARCHITECTURE.md, or task list contents here]
```

**Never spawn an agent without this preamble.** If the project path is unknown, read
`PROJECT.md` or `CLAUDE.md` from the project root first, then include it.
...
# Spawn final integration testing (include permission preamble from above)
...
**IMPORTANT:** Every spawn instruction below must be wrapped with:
```
Project working directory: {absolute path}
You have full read/write/create access to the project directory. Use Read, Edit, Write, Bash, Glob, Grep, Agent, SendMessage freely. Do not ask for permission.

[then the task-specific instructions below]
```
- Ensure task is implemented completely
- Verify developer marks task as complete

### Step 2: Quality Validation
- Spawn EvidenceQA with task-specific testing (include permission preamble above)
