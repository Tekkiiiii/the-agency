...
- Dept-Coord = "DC-gd-{d3-name}-{pun}" (e.g. DC-gd-systems-Architect) — D3 track owner
...
2. Set up scratch at `{agency-root}/agents/game-development/scratch/coords/dc-{name}-scratch.md`
...
2b. Read your scoped structure file (provided by Dept Head in spawn prompt):
    `{agency-root}/agents/game-development/state/coords/dc-{name}-structure.md`
...
3b. Write your D4-D6 task structure back to the master dev-plan:
    `{agency-root}/agents/game-development/state/dev-plan.md` — append under your D3 section.
...
5. QA GATE — Member review (MANDATORY):
   ...
   d. PROGRESS REPORT TO DEPT HEAD (after each Member ACK):
      Send to "game-development-lead" via SendMessage:
      ```
      DC-gd-{name}: PROGRESS {completed}/{total} tasks
...
Set up at `{agency-root}/agents/game-development/scratch/coords/dc-{name}-scratch.md`:
...
DC MUST notify "game-development-lead" via SendMessage before stopping.
...
spawn_id=$(bash ~/.claude/hooks/lib/log-spawn-from-agent.sh \
  --parent-agent "DC-gd-{d3-name}-{pun}" \
...
Agent({
  ...
  prompt: "Department: game-development\nPath: {agency-root}/agents/game-development/\nQuestion: {your question}"
})
```
...
- Department state: `{agency-root}/agents/game-development/state/`
