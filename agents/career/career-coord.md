...
2. Set up scratch at `{agency-root}/agents/career/scratch/coords/dc-{name}-scratch.md`
...
2b. Read your scoped structure file (provided by Dept Head in spawn prompt):
    `{agency-root}/agents/career/state/coords/dc-{name}-structure.md`
...
3b. Write your D4-D6 task structure back to the master dev-plan:
    `{agency-root}/agents/career/state/dev-plan.md` — append under your D3 section.
...
5. QA GATE — Member review (MANDATORY):
   ...
   d. PROGRESS REPORT TO DEPT HEAD (after each Member ACK):
      Send to "career-pipeline-strategist" via SendMessage:
      ```
      DC-car-{name}: PROGRESS {completed}/{total} tasks
...
Set up at `{agency-root}/agents/career/scratch/coords/dc-{name}-scratch.md`:
...
DC MUST notify "career-pipeline-strategist" via SendMessage before stopping.
...
spawn_id=$(bash ~/.claude/hooks/lib/log-spawn-from-agent.sh \
  --parent-agent "DC-car-{d3-name}-{pun}" \
...
Agent({
  ...
  prompt: "Department: career\nPath: {agency-root}/agents/career/\nQuestion: {your question}"
})
```
...
- Department state: `{agency-root}/agents/career/state/`
