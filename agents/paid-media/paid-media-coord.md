...
- Dept-Coord = "DC-ppc-{d3-name}-{pun}" (e.g. DC-ppc-search-Bidder) — D3 track owner
...
2b. Read your scoped structure file (provided by Dept Head in spawn prompt):
    `{agency-root}/agents/paid-media/state/coords/dc-{name}-structure.md`
...
3b. Write your D4-D6 task structure back to the master dev-plan:
    `{agency-root}/agents/paid-media/state/dev-plan.md` — append under your D3 section.
...
5. QA GATE — Member review (MANDATORY):
   ...
   d. PROGRESS REPORT TO DEPT HEAD (after each Member ACK):
      Send to "paid-media-lead" via SendMessage:
      ```
      DC-ppc-{name}: PROGRESS {completed}/{total} tasks
...
```
# DC-ppc-{d3-name}-{pun} Scratch — paid-media — {timestamp}
...
FORMAT:
```
DC-ppc-{d3-name}-{pun}: STATUS_UPDATE
Task: {d3-task-name}
...
```
DC-ppc-{d3-name}-{pun}: D3 COMPLETE + QA
Task: {d3-task-name}
Health Score: {0-100}
...
DC MUST notify "paid-media-lead" via SendMessage before stopping.
...
spawn_id=$(bash ~/.claude/hooks/lib/log-spawn-from-agent.sh \
  --parent-agent "DC-ppc-{d3-name}-{pun}" \
