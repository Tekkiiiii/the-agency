...
2b. Read your scoped structure file (provided by Dept Head in spawn prompt):
    `{agency-root}/agents/marketing/state/coords/dc-{name}-structure.md`
...
3b. Write your D4-D6 task structure back to the master dev-plan:
    `{agency-root}/agents/marketing/state/dev-plan.md` — append under your D3 section.
...
5. QA GATE — Member review (MANDATORY):
   ...
   d. PROGRESS REPORT TO DEPT HEAD (after each Member ACK):
      Send to "marketing-lead" via SendMessage:
      ```
      DC-mkt-{name}: PROGRESS {completed}/{total} tasks
...
DC MUST notify "marketing-lead" via SendMessage before stopping.
...
spawn_id=$(bash ~/.claude/hooks/lib/log-spawn-from-agent.sh \
  --parent-agent "DC-mkt-{d3-name}-{pun}" \
