# Marketing Department

**Call this department when you need to grow an audience, create content, or build brand presence** — viral campaigns, social media management, SEO, growth experiments, app store optimization, or China-market expansion (Xiaohongshu, WeChat, Zhihu, Bilibili, Baidu, Kuaishou, Taobao/Tmall).

**Leader**: Growth Hacker
**Sub-group**: China (8 agents) — see `china/INDEX.md`
**Model tier**: Members = Sonnet, Leaders = Opus

## Members

| Agent | What it does |
|---|---|
| Growth Hacker | Viral loops, conversion funnels, acquisition experiments, growth channels |
| Content Creator | Multi-platform campaigns, editorial calendars, brand storytelling |
| Twitter Engager | Real-time engagement, thought leadership threads, community growth |
| TikTok Strategist | Viral content creation, algorithm optimization, community building |
| Instagram Curator | Visual storytelling, aesthetic development, engagement |
| Reddit Community Builder | Authentic Reddit engagement, value-driven content |
| LinkedIn Content Creator | Thought leadership, personal branding, inbound content |
| Social Media Strategist | Cross-platform campaigns, LinkedIn/Twitter strategy |
| SEO Specialist | Technical SEO, content optimization, organic traffic growth |
| App Store Optimizer | ASO, conversion rate optimization, app discoverability |
| Content Presentation | Marp-based slide decks, pitch decks, speaker decks — Markdown to HTML/PDF/PPTX |
| Carousel Growth Engine | Auto-generates viral carousels from URLs via Gemini, publishes to TikTok/IG |

## China Sub-team

See [china/INDEX.md](china/INDEX.md) — 8 China-market specialists covering Xiaohongshu, WeChat, Zhihu, Bilibili, Baidu SEO, Kuaishou, and e-commerce.

## Content Production Handoff

Marketing owns content **strategy** (what, who, when, where, why). Content Creation owns **execution** (the written artifact). When you need content produced, build a strategic brief and send it to the CCO. See [Content Request Protocol](../runbooks/content-request-protocol.md) for the full workflow.

## Parent Directory

[← Agency Directory](../INDEX.md)
