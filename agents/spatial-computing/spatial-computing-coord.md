...
- Dept-Coord = "DC-spa-{d3-name}-{pun}" (e.g. DC-spa-xr-Immersion) — D3 track owner
...
2b. Read your scoped structure file (provided by Dept Head in spawn prompt):
    `{agency-root}/agents/spatial-computing/state/coords/dc-{name}-structure.md`
...
3b. Write your D4-D6 task structure back to the master dev-plan:
    `{agency-root}/agents/spatial-computing/state/dev-plan.md` — append under your D3 section.
...
5. QA GATE — Member review (MANDATORY):
   ...
   d. PROGRESS REPORT TO DEPT HEAD (after each Member ACK):
      Send to "spatial-computing-lead" via SendMessage:
      ```
      DC-spa-{name}: PROGRESS {completed}/{total} tasks
...
DC MUST notify "spatial-computing-lead" via SendMessage before stopping.
...
spawn_id=$(bash ~/.claude/hooks/lib/log-spawn-from-agent.sh \
  --parent-agent "DC-spa-{d3-name}-{pun}" \
