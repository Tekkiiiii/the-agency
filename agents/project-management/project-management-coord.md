...
2b. Read your scoped structure file (provided by Dept Head in spawn prompt):
    `{agency-root}/agents/project-management/state/coords/dc-{name}-structure.md`
...
3b. Write your D4-D6 task structure back to the master dev-plan:
    `{agency-root}/agents/project-management/state/dev-plan.md` — append under your D3 section.
4. APPROACH GATE — classify each D6 task as TIER_A or TIER_B before spawning. This gate
   ...
   {agency-root}/agents/project-management/scratch/members/member-{id}-{pun}-checkpoint.md
   and polls it (bounded, ~5 min); DC polls the same directory for `Status: AWAITING`
   and replies under `## Reply` with `ACK_APPROACH — proceed` or `REVISE_APPROACH —
...
5. QA GATE — Member review (MANDATORY):
   ...
   d. PROGRESS REPORT TO DEPT HEAD (after each Member ACK):
      Send to "project-management-lead" via SendMessage:
      ```
      DC-pm-{name}: PROGRESS {completed}/{total} tasks
...
```
# DC-pm-{d3-name}-{pun} Scratch — project-management — {timestamp}
...
DC MUST notify "project-management-lead" via SendMessage before stopping.
...
spawn_id=$(bash ~/.claude/hooks/lib/log-spawn-from-agent.sh \
  --parent-agent "DC-pm-{d3-name}-{pun}" \
