...
#### Added
- **`.github/scripts/check-bash32-parse.sh`** — parse mọi script `*.sh` được track bằng bash 3.2 thật (image docker `bash:3.2`, fallback sang `/bin/bash` khi máy là macOS, và **không bao giờ tự bỏ qua**). macOS ship bash 3.2 vĩnh viễn, còn cả hai runner CI đều là bash 5 — nên một cú pháp chỉ bash 3.2 từ chối sẽ vào thẳng main mà không ai cản. Đúng lỗi đó đã từng làm hỏng `spawn-logger.sh`: bash 3.2 ghép cặp backtick khi quét tìm dấu `)` đóng, kể cả bên trong heredoc đã quote, nên số backtick lẻ làm cả file không parse được. Wave 15 sửa bằng escape `\x60` và ghi lại quy tắc trong comment — comment không phải guard.
- **`.github/scripts/check-design-system-sync.js`** — chạy đúng hàm sync mà `agency upgrade` gọi, end-to-end, với ba trạng thái đích (thiếu file / file cũ / file do user tự thêm), rồi regenerate CSS bằng chính `build.js` vừa được sync. Wave 14 mới chỉ verify nhánh này bằng cách đọc source.
...
#### Fixed
- **`spawn-gate.sh` đọc file profile ở một đường dẫn không ai ghi vào.** Bảy hook profile-aware còn lại đọc `{agency-root}/.hook-profile`; riêng hook này đọc `$HOME/.agency/.hook-profile`. `install.sh` deploy template vào root, README và docs đều bảo user `echo minimal > ~/.claude/.hook-profile` — nên cách tắt gate này được ghi trong tài liệu chưa bao giờ có tác dụng, trên mọi nền tảng, kể từ khi hệ profile ra đời. `docs/HOOKS.md` trước đó còn mô tả sự khác biệt này như thể nó bình thường.
- **Dòng chẩn đoán của `cost-tracker.sh` đã chết từ lúc được viết ra.** Cả khối python bị đẩy qua `2>/dev/null`, nuốt luôn dòng `Session cost: ...` — thứ duy nhất hook này từng hiển thị cho con người — cùng với mọi traceback. Giờ stderr ghi vào `metrics/.cost-tracker.err` (đường dẫn cố định, ghi đè mỗi lần chạy nên tự giới hạn dung lượng) và chỉ dòng đúng mục đích được phát lại; traceback vẫn không lọt ra UI.
- **`rescue.sh` / `rescue.ps1` giờ resolve agency root theo đúng thang ưu tiên** (`$AGENCY_HOME` → `$CLAUDE_CONFIG_DIR` → `~/.claude`). Trước đây `rescue.sh` luôn clone vào `~/.claude`: user cài với `AGENCY_HOME=/opt/agency` chạy rescue sẽ nhận thêm một bản clone thứ hai ở thư mục mà bản cài của họ không hề đọc. `rescue.ps1` còn tệ hơn — nó không kiểm tra remote, nên chạy trong **bất kỳ** git repo nào là có thể `git reset --hard origin/main` chính repo đó; giờ đã có bước xác minh theo remote URL như bản bash.
- **6 file `scripts/*.py` giờ dùng bản twin `agency_root()` nhận biết MSYS.** Wave 15 hoãn lại vì cho rằng python native trên Windows không load được đường dẫn script dạng MSYS nên sửa phần đọc env cũng vô ích. Đã **đo thật** trên `windows-latest`: dưới Git Bash thường script load bình thường (MSYS2 convert cả tham số dạng đường dẫn, không chỉ biến môi trường), và chính phần đọc env mới hỏng — `FileNotFoundError: '\d\a\_temp\...\skills'`, đúng lỗi mà các hook đã mắc. Suy luận cũ sai; CI đỏ trước, xanh sau, cùng một assertion.
...
#### Added
- **`.github/scripts/check-bash32-parse.sh`** — parses every tracked `*.sh` with a real bash 3.2 (the `bash:3.2` docker image, a macOS `/bin/bash` fallback, and it **never skips**). macOS ships bash 3.2 forever and both CI runners are bash 5, so a construct only 3.2 rejects reaches main completely unopposed. Exactly that broke `spawn-logger.sh`: bash 3.2 pairs backticks while scanning for the closing `)`, quoted heredoc or not, so an odd number makes the whole file unparseable. Wave 15 fixed it with `\x60` escapes and recorded the rule in a comment — a comment is not a guard.
- **`.github/scripts/check-design-system-sync.js`** — runs the exact sync function `agency upgrade` calls, end to end, across three destination states (absent / stale / user-added), then regenerates the CSS with the **synced** `build.js`. Wave 14 could only verify that path by reading the source.
...
#### Fixed
- **`spawn-gate.sh` read its profile from a path nothing writes.** The other seven profile-aware hooks read `{agency-root}/.hook-profile`; this one read `$HOME/.agency/.hook-profile`. `install.sh` deploys the template to the root and both the README and the docs tell users to `echo minimal > ~/.claude/.hook-profile`, so the documented way to turn this gate off has never worked, on any platform, since the profile system existed. `docs/HOOKS.md` had been documenting the divergence as if it were normal.
- **`cost-tracker.sh`'s diagnostic had been dead since it was written.** The whole python block was piped through `2>/dev/null`, swallowing the `Session cost: ...` line — the only thing this hook ever shows a human — along with any traceback. stderr now goes to `metrics/.cost-tracker.err` (fixed path, overwritten each run, so it is bounded by construction) and only the intended line is re-emitted; tracebacks still stay out of the UI.
- **`rescue.sh` / `rescue.ps1` now resolve the agency root through the documented ladder** (`$AGENCY_HOME` → `$CLAUDE_CONFIG_DIR` → `~/.claude`). `rescue.sh` used to clone to `~/.claude` unconditionally: a user who installed with `AGENCY_HOME=/opt/agency` got a second clone in a directory their install does not read from, and was told it had been rescued. `rescue.ps1` was worse — it verified no remote, so run inside **any** git repository it could `git reset --hard origin/main` on that repository; it now verifies by remote URL the way the bash version always has.
- **All 6 `scripts/*.py` carry the MSYS-aware `agency_root()` twin.** Wave 15 deferred them believing a native Windows python could not load their MSYS-form script path, which would make fixing the env read pointless. Measured on `windows-latest`: under plain Git Bash the script loads fine (MSYS2 converts path-shaped **arguments** as well as environment variables) and it was the env read that broke — `FileNotFoundError: '\d\a\_temp\...\skills'`, the hooks' defect exactly. The inference was wrong; red then green on the same assertion.
