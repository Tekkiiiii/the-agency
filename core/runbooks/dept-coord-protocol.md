...
2. Set up scratch at:
   ~/.agency/agents/{dept}/scratch/coords/dc-{name}-scratch.md
...
Path: `~/.agency/agents/{dept}/scratch/coords/dc-{name}-scratch.md`
...
Update `State` on every transition. Update `## Children` on every child STATUS_UPDATE. The `Updated` column uses HH:MM in your local timezone.
...
Report: ~/.agency/agents/{dept}/qa/qa-d3-{name}-{timestamp}.md
...
Scratch file: ~/.agency/agents/{dept}/scratch/members/member-{id}-scratch.md
...
1. PD creates a briefing file at:
   `~/.agency/agents/{dept}/state/incoming/{slug}-{YYYY-MM-DD}.md`
2. File format:
   ```
   From: PD-{slug}
   Date: {YYYY-MM-DD}
...
4. Dept Head processes, acts (potentially spawning a DC), and writes outcome to both:
   - The department decision log: `~/.agency/agents/{dept}/memory/decisions/`
...
3. PD acts within project scope and writes outcome to both:
   ...
   - The dept head's decision log: `~/.agency/agents/{dept}/memory/decisions/`
...
When creating or updating a protocol or pipeline, ALL steps must be completed before the task is marked done.
...
3. **If cross-dept:**
   ...
   b. For each partner dept: add protocol reference to their dept-lead.md — include the file path and a one-line description
   c. Create the counterpart protocol file in the partner dept's `protocols/` dir if the partner has a distinct role
...
5. **If cross-dept:** notify partner dept heads
...
- Coord agent definition: `core/agents/coord.md`
...
- Dept state format: `{dept}/state/dept-state.md`
